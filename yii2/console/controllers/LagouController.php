...
class LagouController extends Controller{
	...
	public function actionLagou(){//只爬取今天的全部信息
		...
		foreach($keyData as $k=>$v){//循环首页职位
			...
			do{
				...
				foreach($result['content']['result'] as $k2=>$v2){//循环某职位某页所有招聘职位信息
					...
					$ret = $this->genId(1,$id);//利用id生成器生成id
					if(!$ret){
						return false;
					}
		...
		$redis->set('LagouRecord::PositionTotal',0);//初始化
		$b = $redis->get('LagouRecord::TotalLost');
		$redis->set('LagouRecord::TotalLost',0);
		$c = $redis->get('LagouRecord::TotalSuccess');
		$redis->set('LagouRecord::TotalSuccess',0);
		$d = $redis->get('LagouRecord::TotalFail');
		$redis->set('LagouRecord::TotalFail',0);
	...
	public function actionWangluogongcheng(){
		$flag = 1;
		$count = 1;
		$redis = Yii::$app->redis;
		do{
			...
			$pn = $count;
			$data = array('first'=>$bool,'kd'=>$kd,'pn'=>$pn);//传送的数据
			$url = 'http://www.lagou.com/jobs/positionAjax.json?';//第二层页面的爬取		
			curl_setopt($ch,CURLOPT_URL,$url);
			curl_setopt($ch, CURLOPT_POST, 1); 
			curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
			curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
